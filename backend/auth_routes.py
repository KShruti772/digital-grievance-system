...
def citizen_login():
    if request.method == 'POST':
        ...
        # Accept any login credentials - create or retrieve citizen
        citizen = Citizen.query.filter_by(email=email).first()
        
        if not citizen:
            # If citizen doesn't exist, create a new one
            try:
                new_citizen = Citizen(
                ...
                db.session.commit()
                citizen = new_citizen
                print(f"[✓] New citizen created on login: {email}")
            except Exception as e:
                db.session.rollback()
        ...
        session['user_id'] = user.id if user else citizen.id
